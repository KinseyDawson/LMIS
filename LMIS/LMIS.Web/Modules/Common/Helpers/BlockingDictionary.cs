...
/// <summary>
/// Blocking并发字典类
/// 读多写少的场景下性能比ConcurrentDictionary优秀
/// </summary>
...
public class BlockingDictionary<TKey, TValue> : IDictionary<TKey, TValue>
{
    ...
    {
        _dict = new Dictionary<TKey, TValue>(dictionary ?? throw new ArgumentNullException(nameof(dictionary)));
        _locker = new ReaderWriterLockSlim();
    }
    ...
    {
        return TryGetValue(key, out var local) ? local : GetOrAddInternal(key, _ => value);
    }
    ...
    private TValue GetOrAddInternal(TKey key, Func<TKey, TValue> valueFactory)
    {
        ...
        try
        {
            if (_dict.TryGetValue(key, out var local))
                return local;
            _locker.EnterWriteLock();
            try
            {
    ...
    {
        if (TryGetValue(key, out var local))
            return local;
